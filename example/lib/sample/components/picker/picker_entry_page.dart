...
class PickerEntryPage extends StatelessWidget {
  final String _title;
  final List<AuPickerEntity> dataList = [];
    ...
    return Scaffold(
        ...
        body: ListView(
          children: <Widget>[
            ...
            ListItem(
              title: "Picker/多选/点选（AuMultiSelectTagsPicker）",
              describe: "底部多选弹框",
              onPressed: () {
            ...
            ListItem(
              title: "Picker/多选/点选（AuSelectTagsWithInputPicker）",
              describe: "底部多选弹框带输入框",
              onPressed: () {
            ...
            ListItem(
              ...
              onPressed: () {
                rootBundle.loadString('assets/list_picker.json').then((data) {
                  List<AuPickerEntity> _selectionData = []..addAll(
                      (JsonDecoder().convert(data)["data"]['list'] as List? ??
                              [])
                          .map((o) => AuPickerEntity.fromMap(o)));
  ...
  void _showBottomMultiSelectPicker(BuildContext context) {
    List<AuMultiSelectBottomPickerItem> items = [];
    items.add(new AuMultiSelectBottomPickerItem("100", "这里是标题1"));
    items.add(new AuMultiSelectBottomPickerItem("101", "这里是标题2"));
    items.add(
        new AuMultiSelectBottomPickerItem("102", "这里是标题3", isChecked: true));
    items.add(
        new AuMultiSelectBottomPickerItem("103", "这里是标题4", isChecked: true));
    items.add(new AuMultiSelectBottomPickerItem("104", "这里是标题5"));
    items.add(new AuMultiSelectBottomPickerItem("104", "这里是标题6"));
    AuMultiSelectListPicker.show(
      context,
      items: items,
      pickerTitleConfig: AuPickerTitleConfig(titleContent: "多选 Picker"),
      onSubmit: (List<AuMultiSelectBottomPickerItem> data) {
        var str = "";
        data.forEach((item) {
  ...
  void _showExpandBottomMultiSelectPicker(BuildContext context) {
    ...
    AuMultiSelectListPicker.show<ExpendMultiSelectBottomPickerItem>(
      context,
      items: items,
      pickerTitleConfig: AuPickerTitleConfig(titleContent: "多选 Picker"),
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext dialogContext) {
        return StatefulBuilder(builder: (_, setState) {
          return AuMultiSelectListPicker(
            pickerTitleConfig: AuPickerTitleConfig(titleContent: "多选 Picker"),
            items: items,
            onItemClick: (_, index) {
              if (items.where((element) => element.isChecked).length > 3) {
                AuToast.show("选择数目超过了 3 个", context);
            ...
            onCancel: () {
              AuToast.show('自定义 cancel 回调', context);
            },
          );
        });
      },
  ...
  void _showRangePicker(
      BuildContext context, List<AuPickerEntity> _selectionData) {
    _selectionData.forEach((f) => f.configChild());
    var selectionMenuView = AuMultiColumnPicker(
      entity: _selectionData[3],
      defaultFocusedIndexes: [0, -1, -1],
      onConfirm: (Map<String, List<AuPickerEntity>> result, int? firstIndex,
        ...
        AuToast.show(pickResult.toString(), context);
      },
      onEntityTap: (int columnIndex, int rowIndex, AuPickerEntity entity) {
        AuToast.show('$columnIndex + $rowIndex + ${entity.name}', context);
      },
    );
    showModalBottomSheet(
      context: context,
  ...
  void _showRangePicker1(
    ...
    showModalBottomSheet(
      ...
      builder: (_) => StatefulBuilder(builder: (context, setState) {
        return AuMultiColumnPicker(
          pickerTitleConfig: AuPickerTitleConfig(titleContent: titleName),
          entity: _selectionData[3],
          defaultFocusedIndexes: [0, -1, -1],
          onConfirm: (Map<String, List<AuPickerEntity>> result, int? firstIndex,
              int? secondIndex, int? thirdIndex) {
  ...
  void _showMulSelectTagPicker(BuildContext context) {
    ...
    for (int i = 0; i < tags.length; i++) {
      String it = tags[i];
      AuTagItemBean item = AuTagItemBean(name: it, code: it, index: i);
      items.add(item);
    }
    ...
    AuMultiSelectTagsPicker(
      context: context,
      //排列样式 默认 平均分配排序
      layoutStyle: AuMultiSelectTagsLayoutStyle.average,
      //一行多少个 默认4个
      crossAxisCount: 4,
      //最大选中数目 - 不设置 或者设置为0 则可以全选
      maxSelectItemCount: 5,
      onItemClick: (AuTagItemBean onTapTag, bool isSelect) {
        AuToast.show(onTapTag.toString(), context);
      },
      onMaxSelectClick: () {
        AuToast.show('最大数值不能超过5个', context);
      },
      pickerTitleConfig: AuPickerTitleConfig(
        titleContent: '多选标题',
      ),
      tagPickerConfig: AuTagsPickerConfig(
        tagItemSource: items,
        tagTitleFontSize: 12,
      ...
      onConfirm: (value) {
        AuToast.show(value.toString(), context);
      },
      onCancel: () {
        AuToast.show('点击了取消按钮', context);
      },
  ...
  void _showSelectTagsWithTextInputPicker(BuildContext context) {
    ...
    List<AuTagInputItemBean> items = [];
    for (int i = 0; i < tags.length; i++) {
      String it = tags[i];
      AuTagInputItemBean item =
          AuTagInputItemBean(name: it, index: i, needExpend: (i % 2 == 0));
      items.add(item);
    }

    showDialog(
        context: context,
        builder: (_) => AuSelectTagsWithInputPicker(
              title: '这里是标题文字',
              hintText: '请输入',
              ...
              defaultText: "",
              tagPickerConfig: AuTagsInputPickerConfig(
                tagItemSource: items,
                tagTitleFontSize: 12,
