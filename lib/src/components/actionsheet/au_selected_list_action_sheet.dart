...
typedef AuItemTitleBuilder<T> = dynamic Function(int index, T entity);

/// 每一行删除按钮的点击回调。返回值：是否要删除该 entity，如果该 handler 没有实现或者返回 true，则删除
typedef AuItemDeleteCallback<T> = bool Function(int deleteIdx, T deleteEntity);

/// 视图隐藏时的回调，会把是否是清空按钮触发的销毁视图回传
typedef AuListDismissCallback = void Function(bool isClosedByClearButton);

/// 监听数据刷新和列表关闭操作
class AuSelectedListActionSheetController extends ChangeNotifier {
  /// 是否刷新数据
...
class AuSelectedListActionSheet<T> {
  /// 用来获取 Overlay
  ...
  /// 获取对应 index 行内容的回调。类型必须为 String 或者自定义的 widget.自定义 widget 时，左边的 icon 会自动隐藏，自定义widget填充整行。
  final AuItemTitleBuilder<T> itemTitleBuilder;

  /// 控制视图隐藏/刷新列表等方法
  final AuSelectedListActionSheetController? controller;
  ...
  /// 每一行删除按钮的点击回调。返回值：是否要删除该 entity，如果该 handler 没有实现或者返回 true，则删除
  final AuItemDeleteCallback<T>? onItemDelete;

  /// 视图显示时的回调
  final VoidCallback? onListShowed;

  /// 视图隐藏时的回调，会把是否是清空按钮触发的销毁视图回传
  final AuListDismissCallback? onListDismissed;
  ...
  /// create AuSelectedListActionSheet
  AuSelectedListActionSheet(
  ...
  void _innerShow(bool showByKey) {
    ...
    AuSelectedListActionSheetController? tempController = controller;
    if (tempController == null) {
      tempController = AuSelectedListActionSheetController();
      tempController._isHidden = false;
    }
    _AuActionSheetSelectedItemListContentWidget content =
        _AuActionSheetSelectedItemListContentWidget<T>(
      itemWidget: this,
      onDismiss: (isClear) {
...
class _AuActionSheetSelectedItemListContentWidget<T> extends StatefulWidget {
  final AuSelectedListActionSheet itemWidget;
  ...
  _AuActionSheetSelectedItemListContentWidget(
  ...
  State<StatefulWidget> createState() =>
      _AuActionSheetSelectedItemListState<T>();
}

class _AuActionSheetSelectedItemListState<T>
    extends State<_AuActionSheetSelectedItemListContentWidget<T?>>
    with TickerProviderStateMixin {
  bool _isClosedByClear = false;
  AuSelectedListActionSheetController? _controller;
  ...
  void _onClearAction() {
    if (widget.itemWidget.onClear == null) {
      ...
      AuDialogManager.showConfirmDialog(context,
          title: AuIntl.of(context).localizedResource.confirmClearSelectedList,
          cancel: AuIntl.of(context).localizedResource.cancel,
          confirm: AuIntl.of(context).localizedResource.ok, onConfirm: () {
  ...
  Widget build(BuildContext context) {
    ...
    String title =
        (widget.itemWidget.title != null && widget.itemWidget.title!.isNotEmpty)
            ...
            : AuIntl.of(context).localizedResource.selectedList;
    ...
    if (!widget.itemWidget.isClearButtonHidden) {
      Widget clearWidget = GestureDetector(
        ...
        child: Padding(
            ...
            child: Text(AuIntl.of(context).localizedResource.clear,
