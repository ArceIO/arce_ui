...
typedef AuShareActionSheetItemClickCallBack = void Function(
    int section, int index, AuShareItem shareItem);

/// 点击事件拦截回调（如果配置了此项，返回值为是否拦截，如果为true，则进行拦截，不进行默认回调）
/// section为所在行数（0或1），index是在第几位（从0开始记）,shareItem为渠道相关信息
typedef AuShareActionSheetOnItemClickInterceptor = bool Function(
    int section, int index, AuShareItem shareItem);

/// 分享元素
class AuShareItem extends Object {
  /// 分享类型（参考AuShareItemConstants中的枚举，如果此项不为自定义，则自定义名称和图标不生效）
  int shareType;
...
class AuShareActionSheet extends StatelessWidget {
  /// 第一行渠道列表
  final List<AuShareItem>? firstShareChannels;

  /// 第二行渠道列表
  final List<AuShareItem>? secondShareChannels;
  ...
  /// 点击事件回调
  final AuShareActionSheetItemClickCallBack? clickCallBack;

  /// 点击事件拦截回调（如果配置了此项，返回值为是否拦截，如果为true，则进行拦截，不进行默认回调）
  final AuShareActionSheetOnItemClickInterceptor? clickInterceptor;

  const AuShareActionSheet({super.key, 
  ...
  Widget? _configChannelWidget(BuildContext context, int section, int index) {
    ...
    // 判断是否为自定义标题
    title = (channel.shareType == AuShareItemConstants.shareCustom)
        ? (channel.customTitle ?? "")
        : AuIntl.of(context).localizedResource.shareChannels[channel.shareType];
    // 判断是否为自定义，如果不是自定义图标，则判断是否可点击（决定是否使用置灰图标）
    image = (channel.shareType == AuShareItemConstants.shareCustom)
        ? channel.customImage
        : (channel.canClick
            ? BrunoTools.getAssetImage(
                AuShareItemConstants.shareItemImagePathList[channel.shareType])
            : BrunoTools.getAssetImage(AuShareItemConstants
  ...
  Widget _configActionWidgets(BuildContext context) {
    ...
    tiles.add(Container(
      ...
      child: Text(
        mainTitle ?? AuIntl.of(context).localizedResource.shareTo,
        maxLines: 1,
