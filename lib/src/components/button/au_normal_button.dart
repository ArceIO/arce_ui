...
/// 若[AuBigMainButton]、[AuSmallMainButton]、[AuBigOutlineButton]不能满足用户需要
/// 可以直接使用该按钮
///
...
/// 组件的尺寸也会和child一样大。
/// 那就是说：不给[alignment]属性设置值，即使把[AuNormalButton],放到[column]、[ListView]中，
...
/// * [AuSmallOutlineButton], 小边框按钮
///
class AuNormalButton extends StatelessWidget {
  /// 按钮是否可用 默认是true
  final bool isEnable;
  ...
  /// create AuNormalButton
  const AuNormalButton({
    super.key,
    required this.text,
    ...
    this.insertPadding = const EdgeInsets.symmetric(
        vertical: AuButtonConstant.verticalPadding,
        horizontal: AuButtonConstant.horizontalPadding),
    this.decoration,
    this.child,
    this.textStyle,
