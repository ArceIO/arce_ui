...
/// 已选部分箭头状态的枚举值，共四种
enum AuMultipleButtonArrowState {
  /// 箭头不可展开
  cantUnfold,
...
class AuMultipleBottomButton extends StatefulWidget {
  ...
  final void Function(AuMultipleButtonArrowState)? onSelectedButtonTap;
  ...
  /// 暴露给外界设置多选状态的控制器
  final AuMultipleBottomController? bottomController;

  /// create AuMultipleBottomButton
  const AuMultipleBottomButton(
      {super.key,
      this.mainButton,
      this.subButton,
  ...
  @override
  _AuMultipleBottomButtonState createState() => _AuMultipleBottomButtonState();
}

class _AuMultipleBottomButtonState extends State<AuMultipleBottomButton> {
  late AuMultipleBottomController _controller;
  bool _unfoldState = false;

  @override
  void initState() {
    super.initState();
    _controller = widget.bottomController ?? AuMultipleBottomController();
  }
            ...
            child: Row(
              children: <Widget>[
                ...
                Container(
                  ...
                  child: Text(
                    AuIntl.of(context).localizedResource.selectAll,
  ...
  Widget _selectedCountWidget() {
    Image unfoldWidget = BrunoTools.getAssetImageWithColor(
        AuAsset.iconSelectedUpTriangle,
        AuThemeConfigurator.instance.getConfig().commonConfig.brandPrimary);
    Image foldWidget =
        BrunoTools.getAssetImage(AuAsset.iconUnSelectDownTriangle);

    Image cantFoldWidget = BrunoTools.getAssetImageWithColor(
        AuAsset.iconUnSelectDownTriangle, const Color(0XCCCCCCCC));
    ...
    return GestureDetector(
        ...
        setState(() {
          if (widget.onSelectedButtonTap != null) {
            if (_controller.valueNotifier.value.selectedCount == 0) {
              _unfoldState = false;
              widget
                  .onSelectedButtonTap!(AuMultipleButtonArrowState.cantUnfold);
              return;
            }
            _unfoldState = !_unfoldState;
            if (_unfoldState) {
              widget.onSelectedButtonTap!(AuMultipleButtonArrowState.unfold);
            } else {
              widget.onSelectedButtonTap!(AuMultipleButtonArrowState.fold);
            }
          }
        });
      },
      child: Container(
        ...
        child: Row(
          children: <Widget>[
            Text(
              AuIntl.of(context).localizedResource.selected,
              style: const TextStyle(color: Color(0XFF222222), fontSize: 16),
            ),
              ...
              builder: (context, value, _) {
                ...
                if (widget.hasArrow) {
                  if (value.arrowStatus !=
                      AuMultipleButtonArrowState.defaultStatus) {
                    //使用方主动设置箭头状态的时候
                    Widget? arrow;
                    ...
                    _unfoldState =
                        value.arrowStatus == AuMultipleButtonArrowState.fold;
                    value.arrowStatus =
                        AuMultipleButtonArrowState.defaultStatus;
...
/// 多选状态栏的控制器，控制展示数量的多少
class AuMultipleBottomController {
  AuMultipleBottomController({this.initMultiSelectState}) {
  ...
  void setState(
      ...
      bool? subButtonState,
      AuMultipleButtonArrowState? arrowStatus}) {
...
class MultiSelectState {
  ...
  MultiSelectState(
      ...
      this.arrowStatus = AuMultipleButtonArrowState.defaultStatus});
}
