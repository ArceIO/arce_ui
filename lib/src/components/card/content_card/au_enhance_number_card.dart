...
///
/// 该组件需要配合[AuNumberInfoItemModel]使用，AuNumberInfoItemModel是每个元素的单独模型。
///
/// 布局规则
...
///
///
class AuEnhanceNumberCard extends StatelessWidget {
  /// 待展示的信息
  final List<AuNumberInfoItemModel>? itemChildren;
  ...
  /// the theme config of AuEnhanceNumberCard
  final AuEnhanceNumberCardConfig? themeData;

  /// create AuEnhanceNumberCard
  const AuEnhanceNumberCard({
    super.key,
    this.itemChildren,
    this.rowCount = 3,
    this.runningSpace,
  ...
  Widget build(BuildContext context) {
    AuEnhanceNumberCardConfig defaultConfig =
        themeData ?? AuEnhanceNumberCardConfig();

    defaultConfig = defaultConfig.merge(AuEnhanceNumberCardConfig(
  ...
  Widget _buildItemWidget(
      AuNumberInfoItemModel model, AuEnhanceNumberCardConfig config,
      {double? width}) {
    return Column(
  ...
  Widget _buildBottomWidget(
    ...
    if (model.iconTapCallBack != null) {
      icon = BrunoTools.getAssetSizeImage(AuAsset.iconQuestion, 14, 14);

      if (model.numberInfoIcon == AuNumberInfoIcon.arrow) {
        icon = BrunoTools.getAssetSizeImage(AuAsset.iconRightArrow, 14, 14);
      }
  ...
  Widget _getPreWidget(String? preDesc, AuEnhanceNumberCardConfig config) {
  ...
  Widget _getLastWidget(String? lastDesc, AuEnhanceNumberCardConfig config) {
...
class AuNumberInfoItemModel {
  ...
  ///icon的事件
  final Function(AuNumberInfoItemModel)? iconTapCallBack;

  ///icon的样式 可枚举 , 默认为问号
  final AuNumberInfoIcon numberInfoIcon;
  ...
  AuNumberInfoItemModel({
    this.number,
    this.title,
    this.numberInfoIcon = AuNumberInfoIcon.question,
