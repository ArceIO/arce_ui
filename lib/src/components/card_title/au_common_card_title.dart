...
///    subTitleWidget: AuStartRatingIndicator(rating: 4),
///    onTap: () {
...
/// )
/// 相关按钮如下:
///  * [AuActionCardTitle], 右侧为箭头的卡片标题组件
///
class AuCommonCardTitle extends StatelessWidget {
  /// 标题
  final String title;
  ...
  /// the theme config of AuCommonCardTitle
  final AuCardTitleConfig? themeData;

  /// create AuCommonCardTitle
  const AuCommonCardTitle(
      {super.key,
      required this.title,
      this.accessoryText,
  ...
  Widget build(BuildContext context) {
    AuCardTitleConfig defaultConfig = themeData ?? AuCardTitleConfig();

    AuCardTitleConfig cardTitleConfig = AuCardTitleConfig(
        alignment: alignment,
        cardTitlePadding: padding as EdgeInsets?,
        detailTextStyle: AuTextStyle(color: detailColor));
  ...
  Widget _rowWidget(BuildContext context, AuCardTitleConfig defaultConfig) {
  ...
  ///标题最右侧的widget
  Widget _accessoryTextWidget(AuCardTitleConfig defaultConfig) {
    Text tx = Text(
      accessoryText ?? "",
  ...
  ///标题widget
  Widget _titleWidget(BuildContext context, AuCardTitleConfig defaultConfig) {
  ...
  ///标题下方的widget
  Widget _detailTextWidget(AuCardTitleConfig defaultConfig) {
    Text tx = Text(
      detailTextString ?? "",
