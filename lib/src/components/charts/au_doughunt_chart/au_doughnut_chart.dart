...
/// AuDoughnutDataItem 饼状图展示所使用的数据
/// 设置数据的数值、标题、颜色
class AuDoughnutDataItem {
  /// 当前区域值
  final double value;
...
/// 选中扇形区域后执行的回调
typedef AuDoughnutSelectCallback = void Function(
    AuDoughnutDataItem? selectedItem);

class AuDoughnut extends CustomPainter {
  ///圆心位置
  late Offset circleCenter;

  /// 选中的区域
  final AuDoughnutDataItem? selectedItem;

  /// 选中区域回调
  final AuDoughnutSelectCallback? brnDoughnutSelectCallback;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var item in data) {
      ...
      canvas.drawArc(rect, item.startRadius, item.radius, true, paint);
      ...
      if (showTitleWhenSelected == false ||
        ...
        Paint paintIndicator = Paint()
          ...
          ..style = PaintingStyle.fill;
        canvas.drawLine(
            revisedIndicarorLPoint, revisedIndicatorRPoint, paintIndicator);

        /// 画水平线
        Offset indicatorEndOffset = _calcHorizontalOffset(
            revisedIndicarorLPoint, revisedIndicatorRPoint);
        canvas.drawLine(
            revisedIndicatorRPoint, indicatorEndOffset, paintIndicator);
  ...
  bool? hitTest(Offset position) {
    ...
    for (int i = 0; i < length; i++) {
      AuDoughnutDataItem item = data[i];
...
/// AuDoughnutChart 圆形数据展示组件
/// 可选择使用圆形、环形来展示数据所占总数的百分比
class AuDoughnutChart extends StatelessWidget {
  /// 宽度。默认值 0
  final double width;
  ...
  /// 选中项目时候的回掉
  final AuDoughnutSelectCallback? selectCallback;
  ...
  /// create AuDoughnutChart
  const AuDoughnutChart(
      {super.key, this.width = 0,
      this.height = 0,
