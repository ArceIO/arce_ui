import 'package:arce_ui/src/components/charts/au_progress_bar_chart/au_progress_bar_chart_painter.dart';
...
class AuProgressBarChart extends StatefulWidget {
  /// 图表最小宽度，默认0
  ...
  /// 柱形数据，必传
  final List<AuProgressBarBundle> barBundleList;
  ...
  final AuProgressBarChartSelectCallback? barChartSelectCallback;

  /// 图表高度，竖直柱状图有效，默认300
  final double height;

  /// create AuProgressBarChart
  AuProgressBarChart(
      {super.key,
      this.minWidth = 0,
  ...
  State<StatefulWidget> createState() {
    return AuProgressBarChartState();
  }
}

class AuProgressBarChartState extends State<AuProgressBarChart> {
  AuProgressBarItem? _selectedBarItem;
  ...
  Size _chartSize() {
    ...
    if (BarChartStyle.horizontal == widget.barChartStyle) {
      ...
      if (widget.yAxis.axisItemList.isNotEmpty) {
        width += AuProgressBarChartPainter.maxYAxisWidth(widget.yAxis);
      }
      return Size(
  ...
  @override
  void didUpdateWidget(AuProgressBarChart oldWidget) {
    super.didUpdateWidget(oldWidget);
    _clearSelectedBarItem(oldWidget);
  }

  void _clearSelectedBarItem(AuProgressBarChart oldWidget) {
    if (widget.barBundleList.length == oldWidget.barBundleList.length) {
      ...
      for (int bundleIndex = 0; bundleIndex < bundleCount; bundleIndex++) {
        AuProgressBarBundle bundle = widget.barBundleList[bundleIndex];
        AuProgressBarBundle oldBundle = oldWidget.barBundleList[bundleIndex];
        if (bundle.barList.length == oldBundle.barList.length) {
          ...
          for (int barIndex = 0; barIndex < barCount; barIndex++) {
            AuProgressBarItem barItem = bundle.barList[barIndex];
            AuProgressBarItem oldBarItem = oldBundle.barList[barIndex];
  ...
  Widget build(BuildContext context) {
    ...
    if (BarChartStyle.vertical == widget.barChartStyle) {
      double yAxisWidth = AuProgressBarChartPainter.maxYAxisWidth(widget.yAxis);
      return Padding(
        ...
        child: Stack(
          children: <Widget>[
            CustomPaint(
              size: chartSize,
              painter: AuProgressBarChartPainter(
                drawBar: false,
                drawX: false,
                xAxis: widget.xAxis,
                yAxis: widget.yAxis,
