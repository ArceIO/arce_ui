...
typedef BarItemEnumeratorCallback = void Function(
    int barBundleIndex,
    AuProgressBarBundle barBundle,
    int barGroupIndex,
    AuProgressBarItem barItem);

/// 点击柱状数据的回调
typedef AuProgressBarChartSelectCallback = void Function(
    AuProgressBarItem? barItem);
...
/// AuProgressBarBundle 数据图表的数据集
/// 每一个bundle对应一组数据
class AuProgressBarBundle {
  final List<AuProgressBarItem> barList;
...
class AuProgressBarChartPainter extends CustomPainter {
  /// 柱状图的样式
  ...
  final AuProgressBarItem? selectedBarItem;
  final AuProgressBarChartSelectCallback? brnProgressBarChartSelectCallback;
  ...
  static double maxYAxisWidth(ChartAxis yAxis) {
    ...
    if (yAxis.axisItemList.isNotEmpty) {
      //确定y轴文本最大宽度，以及每个文本大小
      for (var item in yAxis.axisItemList) {
  ...
  void _prepareData(Canvas canvas, Size size) {
    ...
    if (0 != barMaxValue) {
      ...
      for (var barbundle in barBundleList) {
        for (var barItem in barbundle.barList) {
    ...
    barItemEnumerator((int barBundleIndex, AuProgressBarBundle barBundle,
        int barGroupIndex, AuProgressBarItem barItem) {
  ...
  void barItemEnumerator(BarItemEnumeratorCallback barItemEnumeratorCallback) {
    ...
    for (int barBundleIndex = 0;
        ...
        barBundleIndex++) {
      AuProgressBarBundle barBundle = barBundleList[barBundleIndex];
      int barNumber = barBundle.barList.length;
      for (int barGroupIndex = 0; barGroupIndex < barNumber; barGroupIndex++) {
        AuProgressBarItem barItem = barBundle.barList[barGroupIndex];
  ...
  @override
  bool shouldRepaint(AuProgressBarChartPainter oldDelegate) {
    return true;
  }
}
