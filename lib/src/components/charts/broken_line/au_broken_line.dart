...
import 'package:arce_ui/src/components/charts/broken_line/au_line_data.dart';
import 'package:arce_ui/src/components/charts/broken_line/au_line_painter.dart';
import 'package:arce_ui/src/components/charts/broken_line/au_line_y_painter.dart';
import 'package:flutter/material.dart';

/// 适用于需要折线图，曲线图的场景
/// 支持数据过多时左右滑动查看数据
/// 支持 X、Y 轴自定义展示文本内容
class AuBrokenLine extends StatefulWidget {
  /// 表格宽高
  final Size size;

  /// 绘制线条的参数内容
  final List<AuPointsLine> lines;
  ...
  /// y轴左侧刻度显示，不传则没有
  final List<AuDialItem>? yDialValues;

  /// x 轴刻度
  final List<AuDialItem>? xDialValues;
  ...
  @override
  State<StatefulWidget> createState() => AuBrokenLineState();
}

class AuBrokenLineState extends State<AuBrokenLine> {
  int pointSelectIndex = -1;
  int lineSelectIndex = -1;

  @override
  Widget build(BuildContext context) {
    var lineWithXPainter = AuLinePainter(
      widget.lines,
            ...
            child: GestureDetector(
              onPanDown: (DragDownDetails e) {
                for (var i = 0; i < widget.lines[0].points.length; i++) {
                  int lineIndex =
                      lineWithXPainter.lineIndexCompute(e.localPosition, i);
                  if (lineIndex >= 0) {
                    lineSelectIndex = lineIndex;
                    pointSelectIndex = i;
                    Point selectedPoint = lineWithXPainter.selectedPoint(
                        lineSelectIndex, pointSelectIndex);
                    _fillLeftTopPoint(
                        widget.lines[lineSelectIndex].points[pointSelectIndex]
                            .lineTouchData,
                        lineWithXPainter.startX,
                        lineWithXPainter.endX,
                        lineWithXPainter.startY,
                        lineWithXPainter.endY,
                        lineWithXPainter.fixedHeight,
                        selectedPoint);
                    setState(() {});
                  }
                }
              ...
              child: Stack(children: [
                CustomPaint(
                  size: widget.size,
                  painter:
                      widget.backgroundColor == null ? lineWithXPainter : null,
                  foregroundPainter:
                      widget.backgroundColor != null ? lineWithXPainter : null,
                  child: widget.backgroundColor != null
                      ? Container(
  ...
  Widget _buildTouchTipWidget(AuPointData pointData) {
    Widget touchTipWidget;
    AuLineTouchData? selectLinePoint = pointData.lineTouchData;
