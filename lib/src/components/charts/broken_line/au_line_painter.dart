...
import 'package:arce_ui/src/components/charts/broken_line/au_base_painter.dart';
import 'package:arce_ui/src/components/charts/broken_line/au_line_data.dart';
import 'package:arce_ui/src/components/charts/broken_line/monotone_x.dart';
import 'package:arce_ui/src/theme/au_theme_configurator.dart';
import 'package:flutter/material.dart';
import 'package:path_drawing/path_drawing.dart';

/// 折线图 刻度线、X/Y 轴绘制类
class AuLinePainter extends AuBasePainter {
  ...
  /// x轴 刻度
  List<AuDialItem>? xDialValues;

  /// y轴左侧刻度显示，不传则没有
  List<AuDialItem>? yDialValues;
  ...
  @override
  bool shouldRepaint(AuLinePainter oldDelegate) {
    return true;
  }
  ...
  void _initValue() {
    ...
    yDialColor ??= AuThemeConfigurator.instance
        .getConfig()
        .commonConfig
        .colorTextSecondary;
    hintLineColor ??=
  ...
  void _drawPointDisplayText(Canvas canvas) {
    if (_linePointPositions.isNotEmpty) {
      for (int lineIndex = 0;
          ...
          lineIndex++) {
        AuPointsLine item = lines[lineIndex];
