...
import 'package:arce_ui/src/components/charts/radar_chart.dart';
...
///   每层layer支持自定义图案绘制，绘制参考[AuFunnelLayerPainter]类定义
class AuFunnelChart extends MultiChildRenderObjectWidget {
  ///漏斗的最大宽度，一般是漏斗最上层的宽度
  ...
  ///用于每层layer的绘制，可以继承[AuFunnelLayerPainter]对每层的绘制进行定制
  final AuFunnelLayerPainter layerPainter;
  ...
  AuFunnelChart.defaultStyle({
    ...
    this.layerMargin = 0,
    this.childOffset = Offset.zero,
  })  : layerPainter = AuDefaultFunnelLayerPainter(),
...
class AuFunnelChartParentData extends ContainerBoxParentData<RenderBox> {}

abstract class RenderFunnelChart extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, AuFunnelChartParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, AuFunnelChartParentData> {
  RenderFunnelChart({
    ...
    required AuFunnelLayerPainter layerPainter,
  ...
  AuFunnelLayerPainter get layerPainter => _layerPainter;

  AuFunnelLayerPainter _layerPainter;

  set layerPainter(AuFunnelLayerPainter value) {
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! AuFunnelChartParentData) {
      child.parentData = AuFunnelChartParentData();
    }
  }
...
///漏斗图RenderObject
class AuFunnelRender extends RenderFunnelChart {
  static const double HALF_PIXEL = 0.5;

  AuFunnelRender({
    required super.layerHeight,
    required super.layerMargin,
    required super.layerCount,
    required super.maxLayerWidth,
    required super.minLayerWidth,
    required super.childOffset,
    bool? gradient,
    List<Color>? layerColors,
    required super.alignment,
    required super.layerPainter,
  ...
  double? getIntrinsicDimensionHorizontal(
    ...
    if (_alignment == MarkerAlignment.center) {
      while (child != null) {
        final AuFunnelChartParentData childParentData =
            child.parentData as AuFunnelChartParentData;
      ...
      while (child != null) {
        ...
        num++;
        AuFunnelChartParentData childParentData =
            child.parentData as AuFunnelChartParentData;
        child = childParentData.nextSibling;
      }
    }
...
///漏斗每层绘制接口类，自定义每层绘制继承该类。示例查看[AuDefaultFunnelLayerPainter]。
abstract class AuFunnelLayerPainter {
  ///该层layer的颜色是否渐变
...
class AuDefaultFunnelLayerPainter extends AuFunnelLayerPainter {
  ///每层漏斗文案TextStyle
  ...
  List<ui.Color> getLayerColors(int layerIndex) {
    if (layerIndex >= AuFunnelChart.defaultLayerColors.length) {
      //超过最大支持的layer层数
      return [Colors.white];
    }
    return [AuFunnelChart.defaultLayerColors[layerIndex]];
  }
