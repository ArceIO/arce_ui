...
///对绘制的效果也支持一定程度上的定制，参考[AuRadarChartDataProvider],[AuRadarChartStyle]。
class AuRadarChart extends MultiChildRenderObjectWidget {
  ...
  ///提供绘制雷达图所需要的数据
  final AuRadarChartDataProvider provider;
  ...
  static const List<AuRadarChartStyle> defaultRadarChartStyles = [
    AuRadarChartStyle(
...
class AuRadarChartParentData extends ContainerBoxParentData<RenderBox> {}
...
class RenderRadarChart extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, AuRadarChartParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, AuRadarChartParentData> {
  ...
  AuRadarChartDataProvider _dataProvider;
  ...
  RenderRadarChart({
    ...
    required AuRadarChartDataProvider provider,
  ...
  AuRadarChartDataProvider get dataProvider => _dataProvider;

  set dataProvider(AuRadarChartDataProvider value) {
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! AuRadarChartParentData) {
      child.parentData = AuRadarChartParentData();
    }
  }
  ...
  double _getIntrinsicDimensionHorizontal(
    ...
    int i = 0;
    while (child != null) {
      final AuRadarChartParentData childParentData =
          child.parentData as AuRadarChartParentData;
  ...
  void _drawRadar(Canvas canvas) {
    ...
    for (int radarIndex = 0; radarIndex < radarCount; radarIndex++) {
      AuRadarChartStyle radarStyle = _dataProvider.getRadarStyle(radarIndex);
      _radarPainter
        ..isAntiAlias = true
...
abstract class AuRadarChartDataProvider {
  ///Return radar charts' count
  int getRadarCount();

  ///Get the radar drawing style of the radar chart according to the specified index.
  AuRadarChartStyle getRadarStyle(int radarIndex);

  ///Get the radar drawing values of the radar chart according to the specified index.
  ///The values determine radar's vertexes.The length must be same as [AuRadarChart.sidesCount].
  ///The value must range from [AuRadarChart.minValue] to [AuRadarChart.maxValue].
...
///The default AuRadarChartDataProvider which using the default designed style.
class DefaultRadarProvider extends AuRadarChartDataProvider {
  ...
  @override
  AuRadarChartStyle getRadarStyle(int radarIndex) {
    if (radarIndex >= AuRadarChart.defaultRadarChartStyles.length) {
      return const AuRadarChartStyle(
        strokeColor: Color(0xff0984f9),
        areaColor: Color(0x1a2196F3),
        dotted: true,
        dotColor: Color(0xff0984f9),
    ...
    return AuRadarChart.defaultRadarChartStyles[radarIndex];
  }
