...
/// /// /// /// /// /// /// /// /// /
class AuAbnormalStateUtils {
  /// 通过状态获取对应空页面widget
  /// status: 页面状态类型为[EmptyState]
  static Widget getEmptyWidgetByState(
      BuildContext context, AbnormalState status,
      {Image? img, AuEmptyStatusIndexedActionClickCallback? action}) {
    if (AbnormalState.getDataFailed == status) {
      return AuAbnormalStateWidget(
        img: img ?? BrunoTools.getAssetImage(AuAsset.noData),
        title: AuIntl.of(context).localizedResource.fetchErrorAndRetry,
        operateTexts: <String>[
          AuIntl.of(context).localizedResource.clickPageAndRetry
        ],
        action: action,
      );
    } else if (AbnormalState.networkConnectError == status) {
      return AuAbnormalStateWidget(
        img: img ?? BrunoTools.getAssetImage(AuAsset.networkError),
        title: AuIntl.of(context).localizedResource.netErrorAndRetryLater,
        operateTexts: <String>[
          AuIntl.of(context).localizedResource.clickPageAndRetry
        ],
        action: action,
...
/// 空页面操作区域按钮的点击回调
/// index: 被点击按钮的索引
typedef AuEmptyStatusIndexedActionClickCallback = void Function(int index);

/// 异常页面展示一般用于网络错误、数据为空的提示和引导
// ignore: must_be_immutable
class AuAbnormalStateWidget extends StatelessWidget {
  /// 图片
  final Image? img;
  ...
  /// 点击事件回调
  final AuEmptyStatusIndexedActionClickCallback? action;
  ...
  AuAbnormalStateConfig? themeData;

  AuAbnormalStateWidget({super.key, 
    this.img,
    this.title,
    this.content,
    ...
    themeData ??= AuAbnormalStateConfig();
