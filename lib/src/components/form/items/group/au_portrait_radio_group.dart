...
typedef AuPortraitRadioGroupOnChanged = void Function(
    AuPortraitRadioGroupOption? oldStr, AuPortraitRadioGroupOption newStr);

/// 纵向放置的单选 radio 视图。选项可为单行字符串，也可是标题+说明两部分。具体参见 [AuPortraitRadioGroupOption]。
/// 选项的标题/子标题文字样式分别通过 [AuFormItemConfig.optionTextStyle] 和 [AuFormItemConfig.subTitleTextStyle] 控
/// 制，可通过主题配置进行修改。
// ignore: must_be_immutable
class AuPortraitRadioGroup extends StatefulWidget {
  /// 录入项内容是否可编辑，false 时全部选项置灰，默认为 true。
  final bool isEdit;

  /// 初始化时默认选中的项，匹配逻辑是 [AuPortraitRadioGroupOption.title] 和 [AuPortraitRadioGroupOption.subTitle] 都相等。
  final AuPortraitRadioGroupOption? selectedOption;

  /// 备选项数组，参数类型为 [AuPortraitRadioGroupOption]
  final List<AuPortraitRadioGroupOption>? options;

  /// 备选项可用状态数组
  final List<bool>? enableList;

  /// 选项变化回调
  final AuPortraitRadioGroupOnChanged? onChanged;
  ...
  AuPortraitRadioGroup.withSimpleList({
    super.key,
    this.isEdit = true,
    ...
    this.isCollapseContent = false,
            ...
            .map((item) => AuPortraitRadioGroupOption(title: item))
            .toList(),
        selectedOption = options.contains(selectedOption)
            ? AuPortraitRadioGroupOption(
  ...
  /// 通过 [AuPortraitRadioGroupOption] 类型的数组 [options]，构造纵向单选视图。
  AuPortraitRadioGroup.withOptions({
    super.key,
    this.isEdit = true,
    this.selectedOption,
    this.options,
  ...
  @override
  AuPortraitRadioGroupState createState() {
    return AuPortraitRadioGroupState();
  }
}

class AuPortraitRadioGroupState extends State<AuPortraitRadioGroup> {
  late AuPortraitRadioGroupOption? _selectedOption;
  ...
  List<Widget> getRadioList(List<AuPortraitRadioGroupOption>? options) {
    List<Widget> result = [];
    AuPortraitRadioGroupOption option;
    if (options == null || options.isEmpty) {
      result.add(Container());
      return result;
    }

    result.add(const AuLine(
      leftInset: 20,
    ));
            ...
            children: <Widget>[
              AuRadioButton(
                ...
                onValueChangedAtIndex: (int position, bool selected) {
                  ...
                  AuPortraitRadioGroupOption? oldValue = _selectedOption;
  ...
  bool isSameOption(
      AuPortraitRadioGroupOption src, AuPortraitRadioGroupOption? dst) {
    if (dst == null) return false;
    return src.title == dst.title && src.subTitle == dst.subTitle;
  }

  TextStyle getOptionTextStyle(AuPortraitRadioGroupOption opt, int index) {
...
/// 纵置单选视图的选项模型。文字样式见 [AuPortraitRadioGroup] 说明。
class AuPortraitRadioGroupOption {
  /// 标题
  String? title;
  ...
  AuPortraitRadioGroupOption({this.title, this.subTitle});
}
