...
typedef AuSearchBarLeadClickCallback = Function(
...
/// updateTextEdit 是暴漏给使用者的更新方法，该方法在组件的实现setState。
/// 比如想要刷新搜索框 就可以直接调用updateTextEdit()
typedef AuSearchBarDismissClickCallback = Function(
    TextEditingController textEditingController, VoidCallback updateTextEdit);

/// 输入框输入变化的监听
typedef AuSearchBarInputChangeCallback = Function(String input);

/// 输入框提交的监听
typedef AuSearchBarInputSubmitCallback = Function(String input);

/// 用于搜索的AppBar
/// 该组件是[AuAppBar]的特例包装，
/// 实现的思路是：将[AuAppBar.title]设置为textField
/// 更多信息 请查看[AuAppBar]
//ignore: must_be_immutable
class AuSearchAppbar extends PreferredSize {
  /// 搜索框的文本输入控制器
  ...
  /// 搜索框的左侧leading
  final AuSearchBarLeadClickCallback? leadClickCallback;

  /// 可以是字符串也可以是widget
  final dynamic leading;

  /// 取消点击的回调
  final AuSearchBarDismissClickCallback? dismissClickCallback;

  /// 输入变化的监听
  final AuSearchBarInputChangeCallback? searchBarInputChangeCallback;

  /// 输入框提交的监听
  final AuSearchBarInputSubmitCallback? searchBarInputSubmitCallback;
  ...
  AuSearchAppbar(
    ...
    themeData = AuThemeConfigurator.instance
        ...
        .appBarConfig
        .merge(themeData)
        .merge(AuAppBarConfig(systemUiOverlayStyle: systemOverlayStyle));
  }

  @override
  Widget get child => AuAppBar(
  ...
  @override
  Size get preferredSize => const Size.fromHeight(AuAppBarTheme.appBarHeight);
  ...
  Widget _createSearchChild(AuAppBarConfig themeData) {
    return Row(
...
class __SearchInputWidgetState extends State<_SearchInputWidget> {
  ...
  void initState() {
    ...
    if (widget.themeData?.systemOverlayStyle.statusBarBrightness ==
      ...
      _defaultDividerColor = AuThemeConfigurator.instance
          .getConfig()
          .commonConfig
          .dividerColorBase;
      _defaultHintTextColor =
          AuThemeConfigurator.instance.getConfig().commonConfig.colorTextHint;
      _defaultInputTextColor = AuThemeConfigurator.instance
          .getConfig()
          .commonConfig
          .colorTextSecondary;
          ...
          child: Row(
            ...
            children: <Widget>[
              Padding(
                ...
                child: BrunoTools.getAssetSizeImage(AuAsset.iconSearch, 16, 16),
              ),
              Expanded(
                child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText: widget.hint ??
                          AuIntl.of(context).localizedResource.inputSearchTip,
                    ),
