...
import 'package:arce_ui/src/components/picker/au_bottom_picker.dart';
import 'package:arce_ui/src/l10n/au_intl.dart';
import 'package:arce_ui/src/theme/au_theme_configurator.dart';
import 'package:flutter/material.dart';

///取消输入事件回调
typedef AuBottomWritePickerClickCallback = Future<void>? Function(
    String? content);

///确认输入事件回调
typedef AuBottomWritePickerConfirmClickCallback = Future<void>? Function(
    BuildContext dialogContext, String? content);

class AuBottomWritePicker extends StatefulWidget {
  /// 弹窗左边自定义文案，默认 '取消'
  ...
  /// 取消输入事件回调
  final AuBottomWritePickerClickCallback? onCancel;

  /// 确认输入内容事件回调
  final AuBottomWritePickerConfirmClickCallback? onConfirm;
  ...
  static void show(BuildContext context,
      ...
      String title = "",
      String? rightTag,
      AuBottomWritePickerClickCallback? onCancel,
      AuBottomWritePickerConfirmClickCallback? onConfirm,
    ...
    showGeneralDialog(
        ...
        pageBuilder: (BuildContext buildContext, Animation<double> animation,
          ...
          final Widget pageChild = AuBottomWritePicker(
            maxLength: maxLength,
            hintText:
                hintText ?? AuIntl.of(context).localizedResource.pleaseEnter,
            leftTag: leftTag ?? AuIntl.of(context).localizedResource.cancel,
            title: title,
            rightTag: rightTag ?? AuIntl.of(context).localizedResource.ok,
            onConfirm: onConfirm,
            onCancel: onCancel,
...
class _BottomWritePickerState extends State<AuBottomWritePicker> {
  ...
  Widget build(BuildContext context) {
    return AuBottomPickerWidget(
      ...
      cancel: widget.leftTag ?? AuIntl.of(context).localizedResource.cancel,
      onConfirmPressed: () {
  ...
  dynamic _buildRightTag(BuildContext context) {
    if (widget.rightTextColor != null) {
      return Text(
        widget.rightTag ?? AuIntl.of(context).localizedResource.ok,
        style: TextStyle(
          fontSize: 16.0,
