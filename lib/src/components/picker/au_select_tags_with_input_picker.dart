...
///提交按钮事件回调
typedef AuTagInputConfirmClickCallback = Future<void>? Function(
    BuildContext dialogContext,
    List<AuTagInputItemBean>? selectedTags,
    String content);

///关闭 picker回调
typedef AuTagInputCancelClickCallBack = void Function(BuildContext context);

/// 底部弹出的标签选择器，支持文本内容输入。支持单选、多选标签。
class AuSelectTagsWithInputPicker extends Dialog {
  ///弹窗自定义标题
  final String title;
  ...
  ///输入内容事件回调
  final AuTagInputConfirmClickCallback? confirm;

  ///关闭 picker 回调
  final AuTagInputCancelClickCallBack? cancelCallBack;
  ...
  /// tags 数据源
  final AuTagsInputPickerConfig tagPickerConfig;

  /// 动态获取标签的展示文本回调
  final SelectTagWithInputValueGetter<AuTagInputItemBean> onTagValueGetter;

  const AuSelectTagsWithInputPicker(
  ...
  Widget build(BuildContext context) {
    return AuSelectTagsWithInputPickerWidget(
      title: title,
      confirm: confirm,
      maxLength: maxLength,
      hintText: hintText ?? AuIntl.of(context).localizedResource.pleaseEnter,
...
class AuSelectTagsWithInputPickerWidget extends StatefulWidget {
  final String? title;
  final AuTagInputConfirmClickCallback? confirm;
  final AuTagInputCancelClickCallBack? cancelCallBack;
  final int? maxLength;
  ...
  final String? defaultText;
  final TextEditingController? textEditingController;
  final AuTagsInputPickerConfig? tagPickerBean;
  final SelectTagWithInputValueGetter<AuTagInputItemBean>? onTagValueGetter;

  const AuSelectTagsWithInputPickerWidget(
      {super.key,
      this.title,
      this.confirm,
      this.cancelCallBack,
  ...
  @override
  _AuSelectTagsWithInputPickerWidgetState createState() =>
      _AuSelectTagsWithInputPickerWidgetState();
}

class _AuSelectTagsWithInputPickerWidgetState
    extends State<AuSelectTagsWithInputPickerWidget>
  ...
  late List<AuTagInputItemBean> _selectedTags;
  late List<AuTagInputItemBean> _sourceTags;
      ...
      body: Container(
        ...
        child: AuPickerClipRRect(
          borderRadius: BorderRadius.only(
            topLeft: Radius.circular(AuThemeConfigurator.instance
                .getConfig()
                .pickerConfig
                .cornerRadius),
            topRight: Radius.circular(AuThemeConfigurator.instance
                .getConfig()
                .pickerConfig
                .cornerRadius),
          ),
  ...
  void _dataSetup() {
    List<AuTagInputItemBean> tagItems = [];
    List<AuTagInputItemBean> tagSelectedItems = [];
    if (widget.tagPickerBean != null) {
      for (AuTagInputItemBean item in widget.tagPickerBean!.tagItemSource) {
        tagItems.add(item);
        //选中的按钮
  ...
  void _clickTag(bool selected, AuTagInputItemBean tagName) {
    if (selected) {
  ...
  bool isCommitBtnEnable() {
    ...
    for (int i = 0; i < _selectedTags.length; i++) {
      AuTagInputItemBean brnTagInputItemBean = _selectedTags[i];
...
class AuTagsInputPickerConfig {
  AuTagsInputPickerConfig(
  ...
  List<AuTagInputItemBean> tagItemSource;
}
