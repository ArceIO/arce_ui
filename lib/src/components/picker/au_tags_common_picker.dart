...
abstract class CommonTagsPicker extends StatefulWidget {
  ...
  void show() {
    showModalBottomSheet(
      ...
      if (type == AuCommonPickBackType.confirm) {
        if (onConfirm != null) {
...
class _CommonPickerState extends State<CommonTagsPicker> {
  ...
  Widget _createHeaderWidget() {
    return AuPickerTitle(
      ...
      onCancel: () {
        Navigator.of(widget.context).pop(AuCommonPickBackType.cancel);
      },
      onConfirm: () {
        Navigator.of(widget.context).pop(AuCommonPickBackType.confirm);
      },
    );
  }
