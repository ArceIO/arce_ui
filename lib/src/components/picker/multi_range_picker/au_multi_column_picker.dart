...
import 'package:arce_ui/src/components/picker/multi_range_picker/au_multi_column_converter.dart';
import 'package:arce_ui/src/components/picker/multi_range_picker/au_multi_column_list.dart';
...
/// [rowIndex] 数据项所在列中的位置
/// [entity] 被点击的数据项
typedef AuOnEntityTap = void Function(
    int columnIndex, int rowIndex, AuPickerEntity entity);
...
typedef AuOnPickerConfirm = void Function(
    Map<String, List<AuPickerEntity>> results,
    int? firstIndex,
    int? secondIndex,
    int? thirdIndex);
...
class AuMultiColumnPicker extends StatefulWidget {
  /// 筛选数据源
  final AuPickerEntity entity;
  ...
  /// 选择数据后回调函数
  final AuOnPickerConfirm? onConfirm;

  /// 选择项目后回调函数
  final AuOnEntityTap? onEntityTap;

  /// 当前选项是否可以被选中:返回 true 可以被选中 false 不可以被选中
  final AuOnSelectEntityInterceptor? canSelectEntryInterceptor;
  ...
  AuMultiColumnPicker(
    ...
    themeData = themeData!
        .merge(AuThemeConfigurator.instance.getConfig().pickerConfig);
  }

  @override
  _AuSelectionGroupViewState createState() => _AuSelectionGroupViewState();
}

class _AuSelectionGroupViewState extends State<AuMultiColumnPicker> {
  static const AuMultiRangeSelConverter defaultConverter =
      AuMultiRangeSelConverter();
  List<AuPickerEntity> _firstList = [];
  List<AuPickerEntity>? _secondList = [];
  List<AuPickerEntity>? _thirdList = [];
  List<AuPickerEntity> _originalSelectedItemsList = [];
  int? _firstIndex;
  int? _secondIndex;
  int? _thirdIndex;

  int get _columnCount {
    return AuMultiColumnPickerUtil.getTotalColumnCount(widget.entity);
  }
  ...
  Widget _listWidget() {
    ...
    if (!BrunoTools.isEmpty(_firstList) &&
      ...
      /// 1、仅有一级的情况
      /// 1.1 一级单选 && 没有自定义范围的情况
      widgets.add(AuMultiColumnListWidget(
          items: _firstList,
          ...
          singleListItemPick:
              (int listIndex, int index, AuPickerEntity entity) {
  ...
  void _confirmButtonClickEvent() {
    if (widget.onConfirm != null) {
      ...
      Map<String, List<AuPickerEntity>> result = defaultConverter
  ...
  void _initData() {
    ...
    for (AuPickerEntity entity in _originalSelectedItemsList) {
  ...
  //设置数据为未选中状态
  void _resetSelectionDatas(AuPickerEntity entity) {
    entity.isSelected = false;
    for (AuPickerEntity subEntity in entity.children) {
  ...
  void _setSecondIndex(int? secondIndex) {
    ...
    if (seconds.length > _secondIndex!) {
      List<AuPickerEntity> thirds = seconds[_secondIndex!].children;
  ...
  int _getInitialSelectIndex(List<AuPickerEntity>? levelList) {
    int index = -1;
    if (levelList == null || levelList.isEmpty) {
      return index;
    }

    for (AuPickerEntity entity in levelList) {
      if (entity.isSelected) {
  ...
  void _resetSelectStatus() {
    ...
    //数据还原
    for (AuPickerEntity commonEntity in _originalSelectedItemsList) {
