...
/// AuMultiColumnPicker相关工具类
class AuMultiColumnPickerUtil {
  /// 筛选项最多不超过三层,故直接写代码判断,本质为深度优先搜索。
  static int getTotalColumnCount(AuPickerEntity? entity) {
    int count = 0;
    AuPickerEntity? rootEntity = entity;
    ...
    if (rootEntity != null && rootEntity.children.isNotEmpty) {
      ...
      for (AuPickerEntity firstLevelEntity in rootEntity.children) {
        if (firstLevelEntity.children.isNotEmpty) {
          count = count > 2 ? count : 2;
          for (AuPickerEntity secondLevelEntity in firstLevelEntity.children) {
  ...
  /// 确定当前 Item 在第几层级
  static int getCurrentColumnIndex(AuPickerEntity? currentItem) {
    int listIndex = -1;
    if (currentItem != null) {
      listIndex = 0;
  ...
  static bool isSelectedCountExceed(AuPickerEntity? entity) {
