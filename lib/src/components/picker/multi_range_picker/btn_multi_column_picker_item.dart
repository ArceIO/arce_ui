...
class AuMultiRangePickerCommonItem extends StatelessWidget {
  final AuPickerEntity item;
  ...
  final ValueChanged<AuPickerEntity>? itemSelectFunction;

  const AuMultiRangePickerCommonItem({super.key, 
    required this.item,
  ...
  String _getSelectedItemCount(AuPickerEntity item) {
    String itemCount = "";
    if ((AuMultiColumnPickerUtil.getTotalColumnCount(item) < 3 ||
        !isFirstLevel)) {
      int count =
  ...
  bool _hasCheckBoxBrother(AuPickerEntity item) {
    int? count;
    if (item.parent != null) {
