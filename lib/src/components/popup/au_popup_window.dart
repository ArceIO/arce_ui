...
class AuPopupWindow extends StatefulWidget {
  /// 依附的组件的 Context
  ...
  /// 依附的组件和 AuPopUpWindow 组件共同持有的 GlobalKey
  ...
  /// popUpWindow 位于 targetView 的方向，默认为 [AuPopupDirection.bottom]
  ...
  const AuPopupWindow(this.context,
      {super.key,
      this.text,
  ...
  /// 显示 popUpWindow
  /// [text] 显示的文本内容
  /// [popKey] 依附的组件和 AuPopUpWindow 组件共同持有的 GlobalKey
  /// [popDirection] 箭头的方向
  ...
  static void showPopWindow(context, String? text, GlobalKey popKey,
      {AuPopupDirection popDirection = AuPopupDirection.bottom,
  ...
  @override
  _AuPopupWindowState createState() => _AuPopupWindowState();
}

class _AuPopupWindowState extends State<AuPopupWindow> {
  /// targetView的位置
  ...
  /// 箭头展示方向
  late AuPopupDirection _popDirection;
        ...
        ? Positioned(
            left: widget.arrowOffset ??
                _left +
                    ...
                    widget.spaceMargin,
            top: _popDirection == AuPopupDirection.bottom
                ? _top - widget.arrowHeight
                : null,
            bottom: _popDirection == AuPopupDirection.top
                ? _bottom - widget.arrowHeight
                : null,
            child: CustomPaint(
              size: Size(15.0, widget.arrowHeight),
              painter: _TrianglePainter(
                  isDownArrow: _popDirection == AuPopupDirection.top,
  ...
  Widget _buildPopWidget(BuildContext context) {
    ...
    return Positioned(
        ...
        top: _popDirection == AuPopupDirection.bottom ? _top : null,
        bottom: _popDirection == AuPopupDirection.top ? _bottom : null,
        child: Container(
            ...
            constraints: BoxConstraints(
                ...
                maxHeight: _popDirection == AuPopupDirection.bottom
...
class AuPopupRoute extends PopupRoute {
  final Duration _duration = const Duration(milliseconds: 200);
  Widget child;

  AuPopupRoute({required this.child});
...
typedef AuPopupListItemClick = bool Function(int index, String item);

/// popup 用于构造自定义的 Item
/// [index] Item 的索引
/// [item] Item 内容
typedef AuPopupListItemBuilder = Widget? Function(int index, String item);

/// 基于 PopUpWindow 的 弹窗列表工具类
class AuPopupListWindow {
  /// 带 itemBuilder 的 Popup List Window
  /// [popKey] 依附的组件和AuPopUpWindow组件共同持有的GlobalKey
  /// [data] 要显示的文本数据列表
  /// [popDirection] 箭头的方向
  ...
  static void showButtonPanelPopList(
    ...
    List<String>? data,
    AuPopupDirection popDirection = AuPopupDirection.bottom,
    AuPopupListItemBuilder? itemBuilder,
    AuPopupListItemClick? onItemClick,
  ...
  static void showPopListWindow(context, GlobalKey popKey,
    ...
    double maxHeight = 200;
    Color borderColor =
        AuThemeConfigurator.instance.getConfig().commonConfig.dividerColorBase;
    Color backgroundColor = Colors.white;
    TextStyle textStyle = TextStyle(
        color:
            AuThemeConfigurator.instance.getConfig().commonConfig.colorTextBase,
  ...
  static double _getMaxWidth(TextStyle textStyle, List<String> data) {
    ...
    if (!BrunoTools.isEmpty(data)) {
      ...
      for (String entity in data) {
        Size size = AuTextUtil.textSize(entity, textStyle);
  ...
  static Widget _getTextWidget(AuPopupListItemBuilder? itemBuilder,
