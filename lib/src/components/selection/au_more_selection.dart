...
import 'package:arce_ui/src/components/selection/widget/au_selection_more_item_widget.dart';
...
///
///
/// 参考[AuSelectionEntity]和[AuSelectionView]
class AuMoreSelectionPage extends StatefulWidget {
  final AuSelectionEntity entityData;
  final Function(AuSelectionEntity)? confirmCallback;
  final AuOnCustomFloatingLayerClick? onCustomFloatingLayerClick;
  final AuSelectionConfig themeData;

  const AuMoreSelectionPage(
      {super.key,
  ...
  @override
  _AuMoreSelectionPageState createState() => _AuMoreSelectionPageState();
}

class _AuMoreSelectionPageState extends State<AuMoreSelectionPage>
  ...
  void checkValue(AuSelectionEntity entity) {
    clearSelectedEntity();
  }

  void clearSelectedEntity() {
    List<AuSelectionEntity> tmp = [];
    AuSelectionEntity node = widget.entityData;
    tmp.add(node);
    while (tmp.isNotEmpty) {
      node = tmp.removeLast();
      if (node.isSelected &&
          (node.filterType == AuSelectionFilterType.range ||
              node.filterType == AuSelectionFilterType.dateRange ||
              node.filterType == AuSelectionFilterType.dateRangeCalendar)) {
        if (node.customMap != null &&
            !BrunoTools.isEmpty(node.customMap!['min']) &&
            !BrunoTools.isEmpty(node.customMap!['max'])) {
          if (!node.isValidRange()) {
            isValid = false;
            if (node.filterType == AuSelectionFilterType.range) {
              AuToast.show(AuIntl.of(context).localizedResource.enterRangeError,
                  context);
            } else if (node.filterType == AuSelectionFilterType.dateRange ||
                node.filterType == AuSelectionFilterType.dateRangeCalendar) {
              AuToast.show(AuIntl.of(context).localizedResource.enterRangeError,
                  context);
            }
...
class MoreBottomSelectionWidget extends StatelessWidget {
  ...
  final Function(AuSelectionEntity)? conformCallback;
  final AuSelectionConfig themeData;
