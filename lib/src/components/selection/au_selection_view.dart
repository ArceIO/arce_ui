...
typedef AuConfigTagCountPerRow = Function(int index, AuSelectionEntity entity);

/// [menuTitle] 设置自定义 menu 的Title文案
/// [isMenuTitleHighLight] 设置自定义 menu 的 title 是否高亮
typedef AuSetCustomSelectionMenuTitle = void Function(
    {String menuTitle, bool isMenuTitleHighLight});

typedef AuOnSelectionChanged = void Function(
    int menuIndex,
    Map<String, String> selectedParams,
    Map<String, String> customParams,
    AuSetCustomSelectionMenuTitle setCustomMenuTitle);

/// menu 点击拦截回调
/// [index] menu 的索引位置
/// 返回 true 拦截点击方法，false 不拦截
typedef AuOnMenuItemInterceptor = bool Function(int index);

/// 筛选弹窗打开前的回调方法。
typedef AuOnSelectionPreShow = AuSelectionWindowType Function(
    int index, AuSelectionEntity entity);

/// 点击【更多】筛选项时的回调，
/// [index] 为点击的位置，
/// [openMorePage] 为让用户触发的回调用于展开更多筛选页面
typedef AuOnMoreSelectionMenuClick = void Function(
    int index, AuOpenMorePage openMorePage);

/// 打开更多筛选页面，
/// [updateData] 是否要更新更多筛选的数据，
/// [moreSelections] 最新的更多筛选数据，是否更新取决于 [updateData]
typedef AuOpenMorePage = void Function(
    {bool updateData, List<AuSelectionEntity> moreSelections});

/// 自定义类型的 Menu 被点击时 让外部设置选中的 value 进来统一更新 UI，并将 function 传给外部设置筛选值。
typedef AuSetCustomSelectionParams = void Function(
    Map<String, String> customParams);

/// 自定义类型的 menu 被点击的回调，
/// [index] 点击位置，
/// [customMenuItem] 自定义筛选 menu 原始数据，
/// [customSelectionParams] 开放给外部回调给函数，用于更新自定义筛选参数，触发[AuOnSelectionChanged]。
typedef AuOnCustomSelectionMenuClick = Function(
    int index,
    AuSelectionEntity customMenuItem,
    AuSetCustomSelectionParams customSelectionParams);

/// 当更多筛选页面中，类型为 CustomLayer 被回调时，该函数用于回传参数进 AuSelectionView 中，
/// [customParams] 用户自定义参数。
typedef AuSetCustomFloatingLayerSelectionParams = void Function(
    List<AuSelectionEntity> customParams);

/// 类型为 CustomLayer 被点击的回调
/// [index] 被点击的位置
/// [customFloatingLayerEntity] 被点击 index 位置的筛选数据
/// [setCustomFloatingLayerSelectionParams] 外部自定义参数回传函数
typedef AuOnCustomFloatingLayerClick = Function(
    int index,
    AuSelectionEntity customFloatingLayerEntity,
    AuSetCustomFloatingLayerSelectionParams
...
const AuSelectionConverterDelegate _defaultConverter =
    DefaultSelectionConverter();

/// 筛选组件
// ignore: must_be_immutable
class AuSelectionView extends StatefulWidget {
  final AuSelectionConverterDelegate selectionConverterDelegate;
  final AuOnCustomSelectionMenuClick? onCustomSelectionMenuClick;
  final AuOnCustomFloatingLayerClick? onCustomFloatingLayerClick;
  final AuOnMoreSelectionMenuClick? onMoreSelectionMenuClick;
  final AuOnSelectionChanged onSelectionChanged;
  final List<AuSelectionEntity> originalSelectionData;
  final AuOnMenuItemInterceptor? onMenuClickInterceptor;
  final AuOnSelectionPreShow? onSelectionPreShow;
  ...
  AuSelectionViewController? selectionViewController;
  ...
  @override
  State<StatefulWidget> createState() => AuSelectionViewState();
}

class AuSelectionViewState extends State<AuSelectionView> {
  final Map<String, String> _customParams = {};
  AuSelectionViewController? _selectionViewController;

  @override
  void initState() {
    super.initState();
    _selectionViewController =
        widget.selectionViewController ?? AuSelectionViewController();
    ...
    if (widget.originalSelectionData.isNotEmpty) {
      ...
      return AuSelectionMenuWidget(
        ...
        onMenuItemClick: (int menuIndex) {
          ...
          if (widget.originalSelectionData[menuIndex].filterType ==
                  AuSelectionFilterType.more &&
              widget.onMoreSelectionMenuClick != null) {
            widget.onMoreSelectionMenuClick!(menuIndex, (
                {bool updateData = false,
                List<AuSelectionEntity>? moreSelections}) {
              if (updateData) {
                List<AuSelectionEntity> moreSelectionEntities =
                    moreSelections ?? [];
        ...
        onConfirm: (AuSelectionEntity results, int firstIndex, int secondIndex,
            int thirdIndex) {
  ...
  void _openMore(AuSelectionEntity entity,
      {AuOnCustomFloatingLayerClick? onCustomFloatingLayerClick}) {
    if (entity.children.isNotEmpty) {
      Navigator.of(context).push(PageRouteBuilder(
          opaque: false,
          pageBuilder: (context, animation, second) {
            return AuMoreSelectionPage(
              entityData: entity,
              themeData: widget.themeData!,
