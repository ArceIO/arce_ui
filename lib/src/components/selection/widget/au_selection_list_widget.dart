...
import 'package:arce_ui/src/components/selection/widget/au_selection_single_list_widget.dart';
...
typedef SingleListItemSelect = void Function(
    int listIndex, int index, AuSelectionEntity entity);

/// 多列数据选择子组件，内部包含多个 AuSelectionSingleListWidget
// ignore: must_be_immutable
class AuListSelectionGroupWidget extends StatefulWidget {
  /// 筛选数据
...
class _AuSelectionGroupViewState extends State<AuListSelectionGroupWidget> {
  final int maxShowCount = 6;

  List<AuSelectionEntity> _firstList = [];
  List<AuSelectionEntity> _secondList = [];
  List<AuSelectionEntity> _thirdList = [];
  ...
  List<Widget> _configWidgets() {
    ...
    // TODO 判断是否添加 Bottom
    if (totalLevel == 1 &&
        widget.entity.filterType == AuSelectionFilterType.radio) {
    } else {
  ...
  Widget _listWidget() {
    ...
    if (!BrunoTools.isEmpty(_firstList) &&
      ...
      /// 1、仅有一级的情况
      /// 1.1 一级单选 && 没有自定义范围的情况
      widgets.add(AuSelectionSingleListWidget(
          items: _firstList,
  ...
  ///还原数据选中状态
  void _resetSelectionData(AuSelectionEntity entity) {
  ...
  void _setSecondIndex(int secondIndex) {
    ...
    if (seconds.length > _secondIndex) {
      List<AuSelectionEntity> thirds = seconds[_secondIndex].children;
  ...
  int _getInitialSelectIndex(List<AuSelectionEntity> levelList) {
    int index = -1;
    if (levelList.isEmpty) {
      return index;
    }

    for (AuSelectionEntity entity in levelList) {
      if (entity.isSelected) {
    ...
    if (index < 0) {
      for (AuSelectionEntity entity in levelList) {
        if (entity.isUnLimit() &&
            AuSelectionUtil.getTotalLevel(entity) > 1 &&
  ...
  _processSelectedStatus(AuSelectionEntity entity) {
