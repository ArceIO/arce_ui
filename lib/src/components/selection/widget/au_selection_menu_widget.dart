...
import 'package:arce_ui/src/components/selection/controller/au_selection_view_controller.dart';
import 'package:arce_ui/src/components/selection/widget/au_selection_animate_widget.dart';
import 'package:arce_ui/src/components/selection/widget/au_selection_list_widget.dart';
import 'package:arce_ui/src/components/selection/widget/au_selection_menu_item_widget.dart';
import 'package:arce_ui/src/components/selection/widget/au_selection_range_widget.dart';
...
typedef AuOnMenuItemClick = bool Function(int index);

typedef AuOnRangeSelectionConfirm = void Function(
    AuSelectionEntity results, int firstIndex, int secondIndex, int thirdIndex);

/// 筛选菜单子组件
class AuSelectionMenuWidget extends StatefulWidget {
  /// 筛选菜单数据
  final List<AuSelectionEntity> data;
  ...
  /// 确认回调
  final AuOnRangeSelectionConfirm? onConfirm;

  /// 筛选菜单项点击回调
  final AuOnMenuItemClick? onMenuItemClick;

  /// 每行的 tag 数，用于子组件配置
  final AuConfigTagCountPerRow? configRowCount;
  ...
  @override
  _AuSelectionMenuWidgetState createState() => _AuSelectionMenuWidgetState();
}

class _AuSelectionMenuWidgetState extends State<AuSelectionMenuWidget> {
  bool _needRefreshTitle = true;
  List<AuSelectionEntity> result = [];
  List<String> titles = [];
  List<bool> menuItemActiveState = [];
  List<bool> menuItemHighlightState = [];
  AuSelectionListViewController listViewController =
      AuSelectionListViewController();
  ...
  void initState() {
    ...
    for (AuSelectionEntity parentEntity in widget.data) {
  ...
  OverlayEntry _createEntry(AuSelectionEntity entity) {
  ...
  List<Widget> _configMenuItems() {
    ...
    for (int index = 0; index < titles.length; index++) {
      ...
      itemViewList.add(AuSelectionMenuItemWidget(
        title: titles[index],
        ...
        itemClickFunction: () {
          ...
          if (listViewController.isShow) {
            ...
            if (widget.data[index].filterType != AuSelectionFilterType.more &&
                widget.data[index].filterType !=
                    AuSelectionFilterType.customHandle) {
              /// 创建 筛选组件的的入口
  ...
  bool _isRange(AuSelectionEntity entity) {
    if (AuSelectionUtil.hasRangeItem(entity.children) ||
        entity.filterShowType == AuSelectionWindowType.range) {
      return true;
    }
    var totalLevel = AuSelectionUtil.getTotalLevel(entity);
    if (totalLevel == 1 &&
  ...
  Widget _createRangeView(AuSelectionEntity entity) {
    ...
    return AuRangeSelectionGroupWidget(
      ...
      onSelectionConfirm: (AuSelectionEntity result, int firstIndex,
  ...
  Widget _createSelectionListView(AuSelectionEntity entity) {
    /// 顶层筛选 Tab
    return AuListSelectionGroupWidget(
      entity: entity,
      maxContentHeight: DESIGN_SELECTION_HEIGHT /
          DESIGN_SCREEN_HEIGHT *
          MediaQuery.of(context).size.height,
  ...
  void _onConfirmSelect(AuSelectionEntity entity, AuSelectionEntity result,
  ...
  /// 筛选 Title 展示规则
  String? _getSelectedResultTitle(AuSelectionEntity entity) {
    /// 更多筛选不改变 title.故返回 null
    if (entity.filterType == AuSelectionFilterType.more) {
      return null;
    }
  ...
  String? _getTitle(AuSelectionEntity entity) {
    String? title;
    List<AuSelectionEntity> firstColumn =
        AuSelectionUtil.currentSelectListForEntity(entity);
    ...
    if (firstColumn.isNotEmpty) {
      for (AuSelectionEntity firstEntity in firstColumn) {
        secondColumn
            .addAll(AuSelectionUtil.currentSelectListForEntity(firstEntity));
    ...
    if (firstColumn.isEmpty || firstColumn.length > 1) {
      ...
      if (firstColumn[0].isUnLimit()) {
        title = entity.title;
      } else if (firstColumn[0].filterType == AuSelectionFilterType.range ||
          firstColumn[0].filterType == AuSelectionFilterType.date ||
          firstColumn[0].filterType == AuSelectionFilterType.dateRange ||
        ...
        if (secondColumn.isEmpty || secondColumn.length > 1) {
          ...
          if (secondColumn[0].isUnLimit()) {
            title = firstColumn[0].title;
              ...
              secondColumn[0].filterType == AuSelectionFilterType.date ||
              secondColumn[0].filterType == AuSelectionFilterType.dateRange ||
            ...
            if (thirdColumn.isEmpty || thirdColumn.length > 1) {
              ...
              if (thirdColumn[0].isUnLimit()) {
                title = secondColumn[0].title;
                      ...
                      AuSelectionFilterType.range ||
                  thirdColumn[0].filterType == AuSelectionFilterType.date ||
  ...
  String? _getDateAndRangeTitle(
      List<AuSelectionEntity> list, AuSelectionEntity entity) {
    String? title = '';
    if (!BrunoTools.isEmpty(list[0].customMap)) {
      if (list[0].filterType == AuSelectionFilterType.range) {
        title =
            '${list[0].customMap!['min']}-${list[0].customMap!['max']}(${list[0].extMap['unit']?.toString()})';
      } else if (list[0].filterType == AuSelectionFilterType.dateRange ||
          list[0].filterType == AuSelectionFilterType.dateRangeCalendar) {
        title = _getDateRangeTitle(list);
      }
    } else {
      if (list[0].filterType == AuSelectionFilterType.date) {
  ...
  String _getDateRangeTitle(List<AuSelectionEntity> list) {
    String minDateTime = '';
    String maxDateTime = '';
    ...
    if (list[0].customMap != null &&
      ...
      if (minDate != null) {
        minDateTime = DateTimeFormatter.formatDate(
            minDate, AuIntl.of(context).localizedResource.dateFormatYYYYMMDD);
      }
    }
    if (list[0].customMap != null &&
      ...
      if (maxDate != null) {
        maxDateTime = DateTimeFormatter.formatDate(
            maxDate, AuIntl.of(context).localizedResource.dateFormatYYYYMMDD);
      }
    }
    return '$minDateTime-$maxDateTime';
  }

  String? _getDateTimeTitle(List<AuSelectionEntity> list) {
    String? title = "";
  ...
  void _refreshSelectionMenuTitle(int index, AuSelectionEntity entity) {
    if (entity.filterType == AuSelectionFilterType.more) {
