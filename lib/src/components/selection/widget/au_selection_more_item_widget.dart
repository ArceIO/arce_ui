...
import 'package:arce_ui/src/components/picker/time_picker/date_picker/au_date_picker.dart';
...
import 'package:arce_ui/src/components/selection/widget/au_layer_more_selection_page.dart';
...
class AuMoreSelectionWidget extends StatefulWidget {
  //entity 是商圈、钥匙等
  final AuSelectionEntity selectionEntity;
  ...
  @override
  _AuMoreSelectionWidgetState createState() => _AuMoreSelectionWidgetState();
}

class _AuMoreSelectionWidgetState extends State<AuMoreSelectionWidget> {
  @override
  Widget build(BuildContext context) {
    //弹出浮层
    if (widget.selectionEntity.filterType == AuSelectionFilterType.layer ||
        widget.selectionEntity.filterType ==
            AuSelectionFilterType.customLayer) {
...
class __FilterCommonTypeWidgetState extends State<_FilterCommonTypeWidget> {
  ...
  Widget _buildSelectionTag() {
    return GridView.count(
      ...
      children: widget.selectionEntity
          .currentShowTagByExpanded(isExpanded)
          .map((AuSelectionEntity data) {
        return GestureDetector(
          onTap: () {
            setState(() {
              if (data.filterType == AuSelectionFilterType.radio) {
                data.parent?.clearSelectedEntity();
                data.isSelected = true;
                //用于发送 标签点击事件
                streamController.add(SelectEvent());
              } else if (data.filterType == AuSelectionFilterType.checkbox) {
                if (!data.isSelected) {
                  if (!AuSelectionUtil.checkMaxSelectionCount(data)) {
                    AuToast.show(
                        AuIntl.of(context)
                            .localizedResource
                ...
                data.parent?.children
                    .where((_) => _.filterType == AuSelectionFilterType.radio)
                    .forEach((f) => f.isSelected = false);
                data.isSelected = !data.isSelected;
                //用于发送 标签点击事件
                streamController.add(SelectEvent());
              } else if (data.filterType == AuSelectionFilterType.date) {
  ...
  Widget _buildSingleTag(AuSelectionEntity entity) {
    bool isDate = entity.filterType == AuSelectionFilterType.date;
  ...
  void _showDatePicker(AuSelectionEntity data) {
    int time =
        int.tryParse(data.value ?? "") ?? DateTime.now().millisecondsSinceEpoch;
    AuDatePicker.showDatePicker(context,
        pickerMode: AuDateTimePickerMode.date,
        pickerTitleConfig: AuPickerTitleConfig.Default,
        initialDateTime: DateTime.fromMillisecondsSinceEpoch(time),
        dateFormat: AuIntl.of(context).localizedResource.dateFormatYYYYMMMMDD,
...
class __MoreArrowState extends State<_MoreArrow> {
  ...
  @override
  Widget build(BuildContext context) {
    String asset = isExpanded ? AuAsset.iconUpArrow : AuAsset.iconDownArrow;
...
class _MoreRangeWidget extends StatefulWidget {
  ...
  ///用于自定义的筛选条件 最大值最小值
  final AuSelectionEntity rangeEntity;
...
class __MoreRangeWidgetState extends State<_MoreRangeWidget> {
  ...
  void initState() {
    ...
    minController.addListener(() {
      if (widget.rangeEntity.filterType != AuSelectionFilterType.range) {
        return;
      }
  ...
  @override
  Widget build(BuildContext context) {
    if (widget.rangeEntity.filterType == AuSelectionFilterType.dateRange) {
      return AuSelectionDateRangeItemWidget(
    ...
    return Container(
      child: Center(
        child: TextField(
          ...
          cursorColor: AuThemeConfigurator.instance
              .getConfig()
              .commonConfig
              .brandPrimary,
...
class _FilterLayerTypeWidgetState extends State<FilterLayerTypeWidget> {
          ...
          child: GestureDetector(
            onTap: () {
              if (widget.selectionEntity.filterType ==
                  AuSelectionFilterType.layer) {
                Navigator.of(context)
                    .push(PageRouteBuilder<AuSelectionEntity>(
                        opaque: false,
                        pageBuilder: (context, animation, second) {
                          return AuLayerMoreSelectionPage(
                ...
                if (widget.onCustomFloatingLayerClick != null) {
                  ...
                  widget.onCustomFloatingLayerClick!(
                      ...
                      (List<AuSelectionEntity> customFloatingLayerParams) {
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                SizedBox(
                  height: 16,
                  width: 16,
                  child: BrunoTools.getAssetImage(AuAsset.iconRightArrow),
                )
              ],
            ),
          ),
        ),
        const Padding(padding: EdgeInsets.only(top: 15), child: AuLine())
      ],
    );
  }
  ...
  String _getCondition() {
    ...
    //  1：滤掉不限
    //  2：滤掉有选中的非不限的叶子节点
    List<AuSelectionEntity> result = selectedList.where((value) {
