...
import 'package:arce_ui/src/components/calendar/au_calendar_view.dart';
...
import 'package:arce_ui/src/components/selection/widget/au_selection_date_range_item_widget.dart';
import 'package:arce_ui/src/components/selection/widget/au_selection_menu_widget.dart';
import 'package:arce_ui/src/components/selection/widget/au_selection_range_input_item_widget.dart';
import 'package:arce_ui/src/components/selection/widget/au_selection_range_tag_widget.dart';
...
import 'package:arce_ui/src/utils/au_text_util.dart';
import 'package:arce_ui/src/utils/au_tools.dart';
import 'package:flutter/material.dart';

/// 范围选择子组件
class AuRangeSelectionGroupWidget extends StatefulWidget {
  /// 根节点的筛选数据
  final AuSelectionEntity entity;
  ...
  /// 确认按钮点击事件
  final AuOnRangeSelectionConfirm? onSelectionConfirm;
  ...
  const AuRangeSelectionGroupWidget(
      {super.key,
  ...
  @override
  _AuRangeSelectionGroupWidgetState createState() =>
      _AuRangeSelectionGroupWidgetState();
}

class _AuRangeSelectionGroupWidgetState
    extends State<AuRangeSelectionGroupWidget>
    with SingleTickerProviderStateMixin {
  List<AuSelectionEntity> _originalSelectedItemsList = [];
  List<AuSelectionEntity> _firstList = [];
  List<AuSelectionEntity> _secondList = [];
  int _firstIndex = -1;
  int _secondIndex = -1;
  int totalLevel = 0;
  ...
  Widget build(BuildContext context) {
    totalLevel = AuSelectionUtil.getTotalLevel(widget.entity);
  ...
  Widget _createNewTagAndRangeWidget(
      List<AuSelectionEntity> firstList, Color white) {
    if (firstList.isNotEmpty &&
        AuSelectionUtil.getTotalLevel(widget.entity) == 1) {
      return Column(
        ...
        AuSelectionUtil.getTotalLevel(widget.entity) == 2) {
      var tabBar = AuTabBar(
        tabHeight: 50,
  ...
  List<Widget> _getOneTabContent(AuSelectionEntity filterItem) {
    List<AuSelectionEntity> subFilterList = filterItem.children;

    /// TODO 还要添加 Date  DateRange 类型的判断。
    List<AuSelectionEntity> tagFilterList = subFilterList
        .where((f) =>
            f.filterType != AuSelectionFilterType.range &&
            f.filterType != AuSelectionFilterType.date &&
            f.filterType != AuSelectionFilterType.dateRange &&
            f.filterType != AuSelectionFilterType.dateRangeCalendar)
        .toList();
    Size maxWidthSize = Size.zero;
    for (AuSelectionEntity entity in subFilterList) {
      Size size = AuTextUtil.textSize(entity.title,
    ...
    var tagContainer = tagFilterList.isNotEmpty
        ? Container(
            ...
            child: AuSelectionRangeTagWidget(
                tagWidth: tagWidth,
    ...
    Widget? content;
    for (AuSelectionEntity item in subFilterList) {
      if (item.filterType == AuSelectionFilterType.range) {
        content = AuSelectionRangeItemWidget(
            item: item,
        ...
        break;
      } else if (item.filterType == AuSelectionFilterType.dateRange) {
        content = AuSelectionDateRangeItemWidget(
            item: item,
            minTextEditingController: _minTextEditingController,
            maxTextEditingController: _maxTextEditingController,
            themeData: widget.themeData,
        ...
        break;
      } else if (item.filterType == AuSelectionFilterType.date) {
        DateTime? initialStartDate =
            DateTimeFormatter.convertIntValueToDateTime(item.value);
        DateTime? initialEndDate =
            DateTimeFormatter.convertIntValueToDateTime(item.value);
        content = AuCalendarView.single(
          key: GlobalKey(),
      ...
      } else if (item.filterType == AuSelectionFilterType.dateRangeCalendar) {
        ...
        children: <Widget>[
              ...
              child: Column(
                children: <Widget>[
                  SizedBox(
                    height: 24,
                    width: 24,
                    child: BrunoTools.getAssetImage(AuAsset.iconSelectionReset),
                  ),
                  Text(
                    AuIntl.of(context).localizedResource.reset,
          ...
          Expanded(
            child: AuBigMainButton(
              title: AuIntl.of(context).localizedResource.ok,
              onTap: () {
  ...
  void _confirmButtonClickEvent() {
    ...
    if (totalLevel == 2) {
      List<AuSelectionEntity> subFilterList =
          widget.entity.children[_tabController.index].children;
      List<AuSelectionEntity> selectItems =
    ...
    // 处理Range类型的校验
    AuSelectionEntity? rangeEntity = _getSelectRangeItem(totalLevel == 1
        ? _firstList
        : _firstList[_tabController.index].children);
    if (rangeEntity != null) {
      if (rangeEntity.customMap != null &&
        ...
        if (!rangeEntity.isValidRange()) {
          ...
          if (rangeEntity.filterType == AuSelectionFilterType.range) {
            AuToast.show(
            ...
            AuToast.show(
                AuIntl.of(context).localizedResource.selectRangeError, context);
          }
          return;
        }
      } else {
  ...
  void _setFirstIndex(int firstIndex) {
    ...
    if (widget.entity.children.length > _firstIndex) {
      List<AuSelectionEntity> seconds =
          widget.entity.children[_firstIndex].children;
      if (seconds.isNotEmpty) {
        for (AuSelectionEntity entity in seconds) {
  ...
  void _configDefaultSelectedData() {
    ...
    for (AuSelectionEntity entity in _firstList) {
      if (entity.isSelected) {
    ...
    if (_firstIndex >= 0 && _firstIndex < _firstList.length) {
      ...
      if (_secondList.isNotEmpty) {
        for (AuSelectionEntity entity in _secondList) {
          if (entity.isSelected) {
  ...
  /// 设置数据为未选中状态
  void _resetSelectionDatas(AuSelectionEntity entity) {
  ...
  void _clearNotTagItem(List<AuSelectionEntity> subFilterList) {
    subFilterList
        .where((f) =>
            f.filterType == AuSelectionFilterType.range ||
            f.filterType == AuSelectionFilterType.date ||
  ...
  void _clearTagSelectStatus(List<AuSelectionEntity> subFilterList) {
    subFilterList
        .where((f) => f.filterType != AuSelectionFilterType.range)
        .where((f) => f.filterType != AuSelectionFilterType.date)
        .where((f) => f.filterType != AuSelectionFilterType.dateRange)
        .where((f) => f.filterType != AuSelectionFilterType.dateRangeCalendar)
        .forEach((f) {
      f.isSelected = false;
      f.customMap = <String, String>{};
    });
  ...
  AuSelectionEntity? _getSelectRangeItem(List<AuSelectionEntity> filterList) {
    List<AuSelectionEntity> ranges = filterList
        .where((f) =>
            (f.filterType == AuSelectionFilterType.range ||
                f.filterType == AuSelectionFilterType.dateRange ||
                f.filterType == AuSelectionFilterType.dateRangeCalendar) &&
  ...
  void _resetCustomMapData() {
    for (AuSelectionEntity commonEntity in _originalSelectedItemsList) {
  ...
  int _getInitFocusedIndex(List<AuSelectionEntity> subFilterList) {
  ...
  bool _hasCalendarItem(AuSelectionEntity entity) {
    ...
    /// 查找第二层级
    if (!hasCalendarItem) {
      for (AuSelectionEntity subItem in entity.children) {
