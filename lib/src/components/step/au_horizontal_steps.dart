...
class AuHorizontalSteps extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => AuHorizontalStepsState();
}

class AuHorizontalStepsState extends State<AuHorizontalSteps> {
  Color get _primary {
    return AuThemeConfigurator.instance.getConfig().commonConfig.brandPrimary;
  }
  ...
  @override
  void didUpdateWidget(covariant AuHorizontalSteps oldWidget) {
  ...
  Widget _getIndexIcon(int index) {
    ...
    switch (index) {
      case 1:
        icon = BrunoTools.getAssetSizeImage(AuAsset.iconStep2, 20, 20);
        break;
      case 2:
        icon = BrunoTools.getAssetSizeImage(AuAsset.iconStep3, 20, 20);
        break;
      case 3:
        icon = BrunoTools.getAssetSizeImage(AuAsset.iconStep4, 20, 20);
        break;
      case 4:
        icon = BrunoTools.getAssetSizeImage(AuAsset.iconStep5, 20, 20);
        break;
      default:
        icon = BrunoTools.getAssetSizeImage(AuAsset.iconStepDoing, 20, 20);
        break;
    }
    return icon;
  }
  ...
  Widget _getCompletedIcon(BrunoStep step) {
    ...
    /// 使用组件默认的icon
    return BrunoTools.getAssetSizeImage(AuAsset.iconStepCompleted, 20, 20,
        color: _primary);
  }
  ...
  Widget _getDoingIcon(BrunoStep step) {
    ...
    // 使用组件默认的icon
    return BrunoTools.getAssetSizeImage(AuAsset.iconStepDoing, 20, 20,
        color: _primary);
  }
}
...
class AuStepsController with ChangeNotifier {
  /// 指示当前进行态的步骤
  int currentIndex;
  ...
  AuStepsController({this.currentIndex = 0, this.isCompleted = false});
