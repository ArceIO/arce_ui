...
import 'package:arce_ui/src/components/tabbar/bottom/au_bottom_tab_bar_item.dart';
...
/// tabBar显示状态
enum AuBottomTabBarDisplayType {
  /// 固定显示状态
  fixed,
...
/// 简述：底部导航栏组件容器，其中[AuBottomTabBarItem]控制每个tab的状态
/// 功能：底部导航栏主容器控制，如点击动画，每个tab的悬浮样式
/// 特别注意：默认关闭点击动画，为固定显示状态
class AuBottomTabBar extends StatefulWidget {
  AuBottomTabBar({
    super.key,
    required this.items,
    this.onTap,
    this.currentIndex = 0,
    this.type = AuBottomTabBarDisplayType.fixed,
    this.fixedColor,
    this.iconSize = 24.0,
    ...
    this.isInkResponse = false,
        ...
        assert(
          items.every((AuBottomTabBarItem item) => item.title != null) == true,
  ...
  final List<AuBottomTabBarItem> items;
  ...
  final AuBottomTabBarDisplayType type;
...
/// 底部导航栏中状态控制类
class _BottomTabBarState extends State<AuBottomTabBar>
  ...
  /// 更新整个widget
  @override
  void didUpdateWidget(AuBottomTabBar oldWidget) {
    ...
    if (widget.currentIndex != oldWidget.currentIndex) {
      switch (widget.type) {
        case AuBottomTabBarDisplayType.fixed:
          break;
        case AuBottomTabBarDisplayType.shifting:
  ...
  String _getUnReadText() {
    ...
    try {
      if (item.badgeNo != null) {
        badgeNo = int.parse(item.badgeNo!);
      }
    } catch (e) {
      debugPrint('badgeNo has FormatException');
    }
    return '${badgeNo > item.maxBadgeNo ? '${item.maxBadgeNo}+' : badgeNo}';
  }
