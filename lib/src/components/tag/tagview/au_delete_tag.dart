...
class AuDeleteTag extends StatefulWidget {
  ...
  AuDeleteTag(
      ...
      this.themeData}) {
    themeData ??= AuTagConfig();
    themeData = AuThemeConfigurator.instance
        .getConfig(configId: themeData!.configId)
        .tagConfig
        .merge(themeData);
    themeData = themeData!.merge(AuTagConfig(
        tagBackgroundColor: backgroundColor,
        tagTextStyle: AuTextStyle.withStyle(tagTextStyle)));
  }

  @override
  _AuDeleteTagState createState() => _AuDeleteTagState();
}

class _AuDeleteTagState extends State<AuDeleteTag> {
  late AuDeleteTagController _controller;

  @override
  void initState() {
    super.initState();
    _controller =
        widget.controller ?? AuDeleteTagController(initTags: widget.tags);
  }
    ...
    return Chip(
      ...
      deleteIcon: deleteIconSize != null
          ? BrunoTools.getAssetSizeImage(
              AuAsset.iconClose, deleteIconSize!.width, deleteIconSize!.height,
              color: deleteIconColor)
          : BrunoTools.getAssetImageWithColor(
              AuAsset.iconClose, deleteIconColor),
      onDeleted: () {
        debugPrint('$index');
        didDeleted(index);
      },
...
class AuDeleteTagController {
  ...
  AuDeleteTagController({List<String>? initTags}) {
    _tags = initTags ?? [];
